function donut() {
  ...
  const canvasWidth = 160; // Updated width
  const canvasHeight = 48; // Updated height
  const canvasArea = canvasHeight * canvasWidth;
  const yOffset = 24; // Updated offset
  const xOffset = 80; // Updated offset
  const innerRadius = 2;
  const r1Points = 90; // 90
  const r2Points = 314; // 314
  const fov = 5;

  const what = 60; // Updated size constant
